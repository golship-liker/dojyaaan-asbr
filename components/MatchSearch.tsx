import { Box, IconButton, Menu, MenuItem, Stack, Typography, Autocomplete, TextField } from "@mui/material";
...
const MatchSearch = () => {
  ...
  const onCharacterSelected = (charId: string, playerNumber: 'p1' | 'p2') => {
...
const PlayerSearch = ({
  ...
  return (
    <Autocomplete
      onChange={(event, value) => {onPlayerChanged(playerNumber, value.name)}}
      options={players}
 ...
 const CharacterSelect = ({
   ...
   const handleCharSelect = (playerNumber: 'p1' | 'p2', id?: string) => {
      onCharacterSelected(playerNumber, id);
      handleClose();
   };
       ...
       <Menu anchorEl={anchorEl} open={open} onClose={handleClose} >
         {characters ? (
           characters.map((character) => {
             return (
               <MenuItem key={character.id} onClick={() => handleCharSelect(playerNumber, character.id)}>
                 <Image
